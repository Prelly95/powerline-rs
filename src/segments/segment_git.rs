...
pub fn segment_git(p: &mut Powerline, git_max_size: u8) {
	#[cfg(feature = "flame")]
	...
	let git_max_size = git_max_size as usize;
	let mut branch_string = branch_name.unwrap();
	if git_max_size > 0 && branch_string.chars().count() > git_max_size {
        let mut start = 0;
        for c in branch_string.chars().take(git_max_size) {
            start += c.len_utf8();
        }
        branch_string.drain(start..);
        branch_string.push('…');
    }
	p.segments.push(Segment::new(bg, fg, branch_string));
